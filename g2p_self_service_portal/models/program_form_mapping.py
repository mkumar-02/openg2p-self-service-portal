...
class G2PProgram(models.Model):
    ...
    def update_form_template(self):
        ...
        form_view_id = (
            self.env["website.page"].sudo().search([("id", "=", form_id)]).view_id.id
        )
        form_view_template = (
            self.env["ir.ui.view"].sudo().search([("id", "=", form_view_id)]).arch_db
        )
        ...
        if index != -1:
            if csrf_generate_token_text not in form_view_template:
                form_view_template = (
                    form_view_template[:index]
                    + csrf_generate_token_text
                    + form_view_template[index:]
                )

        self.env["ir.ui.view"].sudo().search([("id", "=", form_view_id)]).write(
            {
